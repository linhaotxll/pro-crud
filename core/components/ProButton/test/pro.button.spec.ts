...
describe('Pro Button', () => {
  ...
  test('button message toast', async () => {
    const sleep = (time: number) => new Promise(r => setTimeout(r, time))
    const handleClickConfirm = vi.fn(() => {
      return sleep(100)
    })
      ...
      setup() {
        const buttonGroup = buildButtonGroup({
          actions: {
            confirm: {
              text: '确认',
              props: { onClick: handleClickConfirm },
              toast: {
                type: 'message',
                loading: { content: 'Loading1 Action' },
                success: { content: 'Success Action' },
              },
            },
          },
        })
    ...
    await expect(wrapper.findAllComponents(Button)[0].vm.$emit('click'))
    await nextTick()

    expect(document.querySelectorAll('.ant-message-loading').length).toBe(1)
    expect(
      document
        .querySelector('.ant-message-loading')!
        .querySelectorAll('span')[1].innerHTML
    ).toBe('Loading1 Action')

    await sleep(150)

    expect(document.querySelectorAll('.ant-message-loading').length).toBe(0)
  ...
  test('button toast is string', async () => {
    const sleep = (time: number) => new Promise(r => setTimeout(r, time))
    const handleClickConfirm = vi.fn(() => {
      return sleep(100)
    })
